...
describe('loadConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'agentree-config-test-'))
  })
  ...
  it('uses defaults for missing keys in partial config', () => {
    writeFileSync(join(tmpDir, '.agentree.json'), JSON.stringify({ install: 'npm ci' }))
    const config = loadConfig(tmpDir)
    expect(config.basePath).toBe('..')
  ...
  it('handles malformed JSON gracefully', () => {
    writeFileSync(join(tmpDir, '.agentree.json'), 'not-json{{{')
    const config = loadConfig(tmpDir)
    expect(config.basePath).toBe('..')
  })
})
