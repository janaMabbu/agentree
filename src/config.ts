...
export interface agent-plexConfig {
  basePath: string
  copy: string[]
  install?: string
...
const DEFAULTS: agent-plexConfig = {
  basePath: '..',
  copy: ['.env', '.env.local'],
  agents: {
    claude: 'claude',
...
export function loadConfig(cwd: string): agent-plexConfig {
  ...
  const file = raw as Partial<agent-plexConfig>
  return {
