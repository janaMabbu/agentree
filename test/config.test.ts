...
describe('loadConfig', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'agentfork-config-test-'))
  })
  ...
  it('uses defaults for missing keys in partial config', () => {
    writeFileSync(join(tmpDir, '.agentfork.json'), JSON.stringify({ install: 'npm ci' }))
    const config = loadConfig(tmpDir)
    expect(config.basePath).toBe('..')
  ...
  it('handles malformed JSON gracefully', () => {
    writeFileSync(join(tmpDir, '.agentfork.json'), 'not-json{{{')
    const config = loadConfig(tmpDir)
    expect(config.basePath).toBe('..')
  })
})
